...
function showbilllist() {
  ...
  for (var i = 0; i < invoices.length; i++) {
    ...
    var row =
      '<tr class="rowShow" onClick="showinfobill(' +
      invoice.id +
      ')">' +
      "<td>" +
      invoice.orderDate +
      ...
      "</td>" +
      "<td>" +
      currency(invoice.finalTotalCost) +
      "</td>" +
      '<td style="color: ' +
      color +
      '">' +
  ...
  for (var i = 0; i < invoices.length; i++) {
    ...
    if (invoice.id == id) {
      ...
      if (invoice.status == "Chưa xử lý") {
        s +=
          "<h4>Tình trạng:</h4>" +
          '<div><span id="status" style="color:red">' +
          invoice.status +
          '</span><label><input type="checkbox" onchange="changeStatus(this,' +
          invoice.id +
          ')" ><span class="slider"></span></label></div>';
      } else {
        s +=
          "<h4>Tình trạng:</h4>" +
          '<div><span id="status" style="color:blue">' +
          invoice.status +
          '</span><label><input type="checkbox" checked onchange="changeStatus(this,' +
          invoice.id +
          ')" ><span class="slider"></span></label></div>';
      }
      s +=
...
function searchBill() {
  ...
  for (var i = 0; i < invoices.length; i++) {
    var invoice = invoices[i];
    var customerNameWithoutAccents = removeAccents(
      invoice.customer.fullName.toLowerCase()
    );
  ...
  for (var i = 0; i < invoicesTemp.length; i++) {
    ...
    row +=
      ...
      ')">' +
      "<td>" +
      invoiceTemp.orderDate +
      "</td>" +
      "<td>" +
      invoiceTemp.customer.fullName +
      "</td>" +
      "<td>" +
      currency(invoiceTemp.finalTotalCost) +
      "</td>" +
      '<td style="color: ' +
      statusColor +
      '">' +
